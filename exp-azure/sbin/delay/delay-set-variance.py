...
dc_delay_variance = config["oneway-delay-variance"]
...
for dc_id in dc_ip_list:
  ...
  dst_delay_variance_table = dc_delay_variance[dc_id]
 
  if not dst_delay_table:
    continue
  if not dst_delay_variance_table:
    continue
  ...
  for ip in ip_list:
    ...
    for dst_dc_id in dst_delay_list:
      ...
      variance = dst_delay_variance_table[dst_dc_id]

      handle += 1
      shell_cmd += "%s 1:1 classid 1:%d htb rate %s;" % \
                  (class_cmd, handle, bandwidth)
      shell_cmd += "%s %d: parent 1:%d netem delay %s %s distribution normal;" % \
                  (delay_cmd, handle, handle, delay, variance)
